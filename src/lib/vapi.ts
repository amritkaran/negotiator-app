...
function createNegotiationPrompt(
  ...
  const context: NegotiationPromptContext = {
    ...
    tripType: (requirements as any).tripType || "one-way",
    waitingTime: (requirements as any).waitingTime,
    tollPreference: (requirements as any).tollPreference,
    specialInstructions: (requirements as any).specialInstructions,
    lowestPriceSoFar,
